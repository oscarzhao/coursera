...
Solution:
```{r correlation-least-square}
exerc.slope <-sd(mlb11$runs)/sd(mlb11$at_bats) * cor(mlb11$runs, mlb11$at_bats)
exerc.intercept <- mean(mlb11$runs) - exerc.slope * mean(mlb11$at_bats)
...
```{r homeruns-vs-runs}
exerc4.m1 <- lm(runs ~ homeruns, data = mlb11)
summary(exerc4.m1)
...
```{r runs-vs-sth-else}
exerc.hits <- lm(runs ~ hits, data = mlb11)
summary(exerc.hits)
...
```{r diag}
exerc.new_obs <- lm(runs ~ new_obs, data = mlb11)
summary(exerc.new_obs)
