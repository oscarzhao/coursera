...
This part is added by myself as exercises on exploring the data.

1. Check whether there is a significant difference in child weight between those whose mothers are smokers and those not. Here I draw a boxplot to display their distribution separately.

```{r plot}
boxplot(weight~habit, data=nc, main= "Distribution of Child Weight by Mother' Smoking Habit", xlab="Mother' Smoking Habbit", ylab="Child Weight (pound)")
```

According to the boxplot, we know that the distribution of child weight of both group is left skewed, but the 
nonsmoke group is heavily skewed.  The two box plots overlap each other, so we cannot conclude there is a difference in the means of child weight.   We need calculate more variables from the data:

```{r summary_weight_by_habit}
weight_habit <- nc %>% group_by(smoke=habit) %>% summarise(n=n(), mean=mean(weight), sd=sd(weight))
weight_habit
```

There is ONE observation who does not provide smoking habit information, we could ignore it due to the large sample size (1000).

Because the data in the two groups are not paired, we shall calculate the difference of means.

```{r}
weight_habit <- weight_habit %>% mutate(seq=sd**2/n) 
diff_mean.se <- sqrt(sum(weight_habit$seq, na.rm = TRUE))
diff_mean.mean <- 7.144 - 6.829
diff_mean.t_score <- diff_mean.mean / diff_mean.se
diff_mean.p_val <- 2*pt(q = diff_mean.t_score, df = 125, lower.tail = F)
diff_mean.p_val
```
p-value < 0.02008 < 0.05, which means we reject H0, in other words, there is a difference in child weights by mother's smoking habit.
...
Answer: B, it's a two-sided test.
...
Answer: D, babies born to nonsmoker mothers are heavier.
...
```{r cutoff-mature}
nc %>% group_by(mature = mature) %>% summarise(min=min(mage), max=max(mage))
```

The age cut-off is 35.
...
Research question: Is there a difference in the average weight of male and female babies?

The null hypothesis is there is no difference in weights of male and female babies. 
...
We run an `inference` to calculate the result:
...
We see that p-value < 0.0001 < 0.05, which strongly reject $H_0$. 
