...
*Answer: 1 - 3/50 = 0.94, not exactly equal, because it's random.*
...
ci <- ci %>% mutate(mean=(lower+upper)/2)
ci <- ci %>% mutate(upper_99=mean + (upper-mean)/z_star_95*z_star_99, lower_99=mean - (mean-lower)/z_star_95*z_star_99)
ci <- ci %>% mutate(capture_mu_99 = ifelse(lower_99 < params$mu & upper_99 > params$mu, "yes", "no"))
ci_data_99 <- data.frame(ci_id = c(1:50, 1:50),
                      ci_bounds = c(ci$lower_99, ci$upper_99),
                      capture_mu = c(ci$capture_mu_99, ci$capture_mu_99))
...
```{r}
dis <- ci %>% summarise(bar_x=mean(mean), sd=sd(mean), n = n())
qnorm(0.995, mean = dis$bar_x, sd = dis$sd/sqrt(dis$n))
qnorm(0.005, mean = dis$bar_x, sd = dis$sd/sqrt(dis$n))
```
