...
Answer: B.  Distinguish mean and median.
...
Answer: C. the more data, the more accurate.
...
sample_means50 %>%
  summarise(mean = mean(x_bar), median = median(x_bar), sd = sd(x_bar))
...
sample_means50 <- ames %>%
  ...
  summarise(pm = mean(price))

ggplot(data=sample_means50, aes(x=pm)) + geom_histogram(binwidth = 1000)
...
ggplot(data=sample_means150, aes(x=pm)) + geom_histogram(binwidth = 1000)
...
ames %>% sample_n(size=15) %>%
  summarise(mean=mean(price), median=median(price), sd=sd(price), size=n(), iqr=IQR(price))
...
sample_means15 <- ames %>%
  ...
  summarise(bar_x_price=mean(price))

sample_means15 %>% summarise(mean=mean(bar_x_price), sd=sd(bar_x_price))
...
sample_means150 %>% summarise(mean=mean(bar_x_price), sd=sd(bar_x_price))
