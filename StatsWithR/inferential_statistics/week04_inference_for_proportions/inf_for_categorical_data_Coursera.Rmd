...
Question: What is **sample statistic**?
Answer: mean, sd, irq, min, max, and generated proportions are **sample statistics**.   So here is True.
...
Answer: The data is collected via three different ways, which we cannot assume samples are randomly selected.  The here is False.
...
```{r perc-atheist-us12}
us12 %>% group_by(response = response) %>% summarise(rate = n()/1002)
...
1. independence (randomly sampled)
2. sucess-failure rules (1002\*0.05 >= 10 and 1002\*0.95 >= 10)
</div>
...
```{r spain-05-12}
isChanged <- function(countryName) {
  us.total <- atheism %>% filter(nationality == countryName) %>%
    group_by(year = year) %>% 
    summarise(total=n())
  
  
  us.non_atheist <- atheism %>% filter(nationality == countryName, response == "non-atheist") %>%
    group_by(year = year) %>% 
    summarise(total=n())
  
  
  us.total <- data.frame(us.total, n_ath = us.total$total - us.non_atheist$total, p_ath = 1 - us.non_atheist$total/us.total$total)
  
  us.p_pool <- sum(us.total$n_ath) / sum(us.total$total)
  us.se_pool <- sqrt(sum(us.p_pool * (1-us.p_pool)/us.total$total))
  
  2*pnorm(abs(us.total$p_ath[2] - us.total$p_ath[1]), mean = 0, sd=us.se_pool, lower.tail = F) <= 0.05
}
...
mySum <- atheism %>% group_by(country=nationality, year = year) %>% summarise(total=n())
myNonAth <- atheism %>% group_by(country=nationality, year = year) %>% filter(response=="non-atheist") %>% summarise(non_atheist=n())
mySum <- merge(mySum, myNonAth, c("country", "year"))  # merge two data.frame into one

myRate <- mySum %>% 
  mutate(rate = 1 - non_atheist/total) %>%
  select(country, year, rate)

myRate2012 <- myRate %>% filter(year=="2012")
myRate2005 <- myRate %>% filter(year=="2005")

myRateMerged <- merge(myRate2005, myRate2012, c("country"))
  

changedCount <- 0
for (countryName in myRateMerged$country) {
  changed <- isChanged(countryName)
  if (changed) {
