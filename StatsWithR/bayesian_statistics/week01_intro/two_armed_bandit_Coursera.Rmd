...
p_goodwin <- 1/2*1/2
p_badwin <- 1/2*1/3
p_bad_win <- p_badwin/(p_badwin + p_goodwin)
p_bad_win # 0.4
```
...
Answer: It's the same question as Q1.
...
```{r bandit_posterior_1}
bandit_posterior(data.frame(machine=c(1L, 1L, 2L, 2L, 2L), outcome=c("W", "W", "W", "W", "L"))) # 3
```
...
```{r bandit_posterior_2}
bandit_posterior(data.frame(machine=c(2L, 2L, 2L, 1L, 1L), outcome=c("W", "W", "L", "W", "W"))) # the same result
```
